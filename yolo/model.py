...
class NetBackbone(tf.keras.models.Model, ABC):
    ...
    def __init__(self, input_shape,use_bias=False, alpha=0.1, beta=16, gama=4):
        super(NetBackbone, self).__init__()
        self.h, self.w, self.c = input_shape
        if gama not in (4,6,8):
            raise ValueError("gama must be in 4,6,8.")
        else:
            self.gama = gama
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.gama in (4,6,8):
            ...
            x = self.block3_residual4(x,training=training)
        if self.gama in (6,8):
            x = self.block3_residual5(x, training=training)
            x = self.block3_residual6(x, training=training)
        if self.gama == 8:
            x = self.block3_residual7(x, training=training)
            x = self.block3_residual8(x, training=training)
        output_1 = x
        ...
        if self.gama in (4,6,8):
            ...
            x = self.block4_residual4(x,training=training)
        if self.gama in (6,8):
            x = self.block4_residual5(x,training=training)
            x = self.block4_residual6(x,training=training)
        if self.gama == 8:
            x = self.block4_residual7(x,training=training)
            x = self.block4_residual8(x,training=training)
        output_2 = x
        x = self.conv_block5(output_2,training=training)
        if self.gama in (4,6):
            x = self.block5_residual1(x,training=training)
            x = self.block5_residual2(x,training=training)
        if self.gama == 8:
            x = self.block5_residual3(x, training=training)
            x = self.block5_residual4(x, training=training)
        output_3 = x
...
class Net(tf.keras.models.Model, ABC):
    def __init__(self,input_shape,out_channels,alpha=0.1,beta=16,gama=4):
        ...
        # Backbone
        self.base_model = NetBackbone(input_shape,beta=self.beta,gama=self.gama)
