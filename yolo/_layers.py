...
class LocalSumLayer(tf.keras.layers.Layer):
    def __init__(self, kernel_size, strides, depth_multiplier=1, use_bias=False):
        ...
        if isinstance(kernel_size, int):
            ...
            self.ks = [1, kernel_size, kernel_size, 1]  # ksize of avg_pool function
        elif isinstance(kernel_size, tuple):
            self.divided_value = float(kernel_size[0] * kernel_size[1])
            self.ks = [1, kernel_size[0], kernel_size[1], 1]
    ...
    def call(self, inputs, **kwargs):
        ...
        x = tf.nn.avg_pool(x, ksize=self.ks, strides=1, padding='SAME') * self.divided_value
        x = tf.keras.layers.concatenate([inputs, x], axis=-1)
        return x


class LocalSumLayerV2(tf.keras.layers.Layer):
    def __init__(self, kernel_size, strides, depth_multiplier=1, use_bias=False):
        super(LocalSumLayerV2, self).__init__()
        if isinstance(kernel_size, int):
            self.divided_value = float(kernel_size * 2)
            self.ks = [1, kernel_size, kernel_size, 1]  # ksize of avg_pool function
        elif isinstance(kernel_size, tuple):
        ...
        self.dw_conv = tf.keras.layers.DepthwiseConv2D(kernel_size=kernel_size,
                                                       ...
                                                       depth_multiplier=depth_multiplier,
                                                       use_bias=use_bias)
        self.freeze_sum = tf.keras.layers.DepthwiseConv2D(kernel_size=kernel_size,
                                                          strides=strides,
                                                          padding='same' if strides == (1, 1) or strides == 1 else 'valid',
                                                          depth_multiplier=depth_multiplier,
                                                          use_bias=use_bias,
