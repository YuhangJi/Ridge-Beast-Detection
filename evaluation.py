...
if __name__ == '__main__':
    ...
    weights_path = r'D:\jiyuhang\new_experiments\release\logs\netepoch-100'
    net = Net(input_shape=(IMAGE_HEIGHT, IMAGE_WIDTH, CHANNELS), out_channels=3 * (CATEGORY_NUM + 5),alpha=0.1)
    net.build()
    net.summary()
    net.load_weights(filepath=weights_path)
    get_map(test_file_path=r'D:\jiyuhang\new_experiments\release\data_process\test.txt',
            gt_file_path=r'D:\jiyuhang\new_experiments\release\mAP\input\ground-truth',
            dr_file_path=r'D:\jiyuhang\new_experiments\release\mAP\input\detection-results',
            model=net)
