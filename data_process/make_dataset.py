...
from config import BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, CATEGORY_NUM, MAX_BOX_NUM
...
def sparseLineData(line_data, is_random=True, is_proc_img=True, jitter=.3, hue=.1, sat=1.5, val=1.5):
...
def sparseBatchData(batch_data):
    batch_image = []
    batch_boxes = []
    for one_line_data in batch_data:
        one_image, one_boxes = sparseLineData(one_line_data)
...
def generateDatasetIterator(txt_dir):
    ds, ds_length = generateDataset(txt_dir)
    ds = ds.repeat()
